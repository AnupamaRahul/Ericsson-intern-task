...
namespace ArrayApplication
{
    class Vector<T> where T : class
    {
        ...
        */
        ...
        public static bool msearch(Vector<T> V, T key)
        {
            for (int i = 0; i < V.nelem; i++)
                if (V.array[i].Equals(key))
                    return true;
            return false;
    ...
    class VectorMain
    {
        static void Main(string[] args)
        {
            ...
            vector.addElement(6);
            vector.addElement(8);
            System.Console.WriteLine(Vector<object>.msearch(vector, 6));
            System.Console.WriteLine(Vector<object>.msearch(vector, 9));
        }
    }
}
